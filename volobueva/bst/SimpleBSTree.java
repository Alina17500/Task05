package ru.vsu.cs.volobueva.bst;
...
//Класс, реализующий простое дерево поиска
public class SimpleBSTree<T extends Comparable<? super T>> extends SimpleBinaryTree<T> implements BSTree<T> {
     ...
     * @throws Exception Если переданое скобочное представление не является деревом поиска
     */
    @Override
    ...
    public void deleteLeavesNotLowLevel(BSTree<Integer> three) {
        class InnerVisitor implements BinaryTreeAlgorithms.Visitor<Integer> {
            final int lowLevel = getLowLevel(three);
            @Override
            public void visit(Integer value, int level) {
                if ((level < lowLevel) && (three.getNode(value).getLeft() == null) && (three.getNode(value).getRight() == null)) {
                    three.remove(value);
                }
            }
        }
        InnerVisitor visitor = new InnerVisitor();
        BinaryTreeAlgorithms.preOrderVisit(three.getRoot(), visitor);

    }

    private int getLowLevel(BSTree<Integer> three) {
        class InnerVisitor implements BinaryTreeAlgorithms.Visitor<Integer> {
            int lowLevel = 0;
            @Override
            public void visit(Integer value, int level) {
